...
test('v11 changes collision timing and preserves the v6 world layout', () => {
  assert.equal(WORLD_VERSION, 11);
...
test('ObservationV4 exposes signed lateral position and 517 values', () => {
  const state = {
    ...createGame('v11-position-contract'),
    fly: { row: 0, column: 3 },
  };
...
test('Reward v5 distinguishes waiting from navigation in environment v10', () => {
  const waiting = stepGame(createGame('v11-wait-cost'), 'wait');
  assert.equal(waiting.reward, -0.04);

  const carriedState = {
    ...createGame('v11-safe-carry'),
    fly: { row: 3, column: 0 },
    score: 3,
    lanes: [{
      row: 3,
