...
class ObservationV1:
    ...
    edge_distance: float
    signed_column: float
    traffic: Sequence[Sequence[float]]
...
def flatten_observation(observation: ObservationV1) -> NDArray[np.float32]:
    """Encode ObservationV4 in the browser policy's stable numeric order."""
    ...
    traffic = [value for row in observation.traffic for value in row]
    encoded = np.asarray(
        [
            *cells,
            *motion,
