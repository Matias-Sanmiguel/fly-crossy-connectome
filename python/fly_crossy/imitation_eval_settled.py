...
@torch.no_grad()
def evaluate(checkpoint_path: Path, episodes: int, max_steps: int, seed_prefix: str, device_name: str) -> dict[str, object]:
    ...
    if validated.connectome_interface != "population-settled" or validated.graph is None:
        raise ValueError("Expected a population-settled connectome checkpoint.")
    model = SettledPopulationFixedGraphPolicy(validated.graph, validated.observation_size, validated.actions).to(device)
    ...
    for episode in range(episodes):
        ...
        hidden = torch.zeros(1, validated.graph.node_count, device=device)
        steps = 0
        while state.terminal is None and steps < max_steps:
            observation = torch.tensor(flatten_observation(observe(state)), dtype=torch.float32, device=device).unsqueeze(0)
    ...
    return {
        ...
        "medianScore": ordered[len(ordered) // 2],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate settled MaleCNS without the expert planner.")
    ...
    print(json.dumps(evaluate(args.checkpoint, args.episodes, args.max_steps, args.seed_prefix, args.device), indent=2))
