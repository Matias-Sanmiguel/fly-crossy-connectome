...
class _PolicyRunner:
    ...
    def reset(self) -> None:
        self.hidden = (
            ...
            if isinstance(self.model, (FixedGraphPolicy, PopulationFixedGraphPolicy, NestedPopulationFixedGraphPolicy))
            else None
        )
...
def _load_checkpoint(
    ...
    if expected_controller == "conventional":
        ...
        if validated.connectome_interface == "nested":
...
def _evidence(
    ...
    *,
    ...
    graph_hash = (
        model.graph.artifact_sha256 if isinstance(model, (FixedGraphPolicy, PopulationFixedGraphPolicy, NestedPopulationFixedGraphPolicy)) else None
    )
    return {
        "checkpointPath": str(checkpoint_path),
        "checkpointSha256": _sha256(checkpoint_path),
        "checkpointController": (
            "connectome" if isinstance(model, (FixedGraphPolicy, PopulationFixedGraphPolicy, NestedPopulationFixedGraphPolicy)) else "conventional"
        ),
...
def evaluate(config_path: str | Path, output: str | Path) -> dict[str, str]:
    ...
    if not isinstance(
        connectome, (FixedGraphPolicy, PopulationFixedGraphPolicy, NestedPopulationFixedGraphPolicy)
    ):
