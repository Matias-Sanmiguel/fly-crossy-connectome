...
- `public/data/connectome/graph.json`: reduced MaleCNS v1.0 graph reused byte-for-byte from Mert Cobanov's `cobanov/flyjump` at commit `c08c86bc18efd8125964b1d2ca4fc1df59700f30`. MaleCNS data is **CC BY 4.0**; compatible application derivation material retains the **Cobanov Template Attribution License 1.0**. See the adjacent notice and manifest. This subset was selected for FlyDino and may be suboptimal here; no Dino game code or assets are included.
...
- `public/assets/kenney/`: nine unmodified low-poly GLB models selected from Kenney's City Kit (Roads), Car Kit, Train Kit, and Mini Forest packs, **CC0 1.0 Universal**. See the bundled `NOTICE.md` and `manifest.json` for exact sources, archive hashes, upstream paths, byte counts, and file hashes. https://kenney.nl/assets/.
